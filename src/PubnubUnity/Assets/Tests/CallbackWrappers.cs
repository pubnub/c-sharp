...
public class CallbackWrappers : PNTestBase {
	...
	public IEnumerator GetFileUrl() {
		pn.GetFileUrl().Channel(channel).FileId("").FileName("").Execute(Callback(out var awaiter, out var assigner));
		yield return awaiter;
		var s = assigner().status;
	...
	public IEnumerator DownloadFile() {
		pn.DownloadFile().Channel(channel).FileName("").FileId("").Execute(Callback(out var awaiter, out var assigner));
		yield return awaiter;
		var s = assigner().status;
		Assert.IsTrue(s.Error);
		Assert.IsTrue(s.ErrorData.Information.Contains("Missing"));
	...
	public IEnumerator SetChannelMetadata() {
		pn.SetChannelMetadata().Channel(channel).Name("testname").Execute(Callback(out var awaiter, out var assigner));
		yield return awaiter;
		var s = assigner().status;
		Assert.IsFalse(s.Error);
	}
	...
	public IEnumerator RemoveAllPushNotificationsFromDeviceWithPushToken() {
		pn.RemoveAllPushNotificationsFromDeviceWithPushToken().Environment(PushEnvironment.Development).Topic("test")
			.Execute(Callback(out var awaiter, out var assigner));
		yield return awaiter;
		var s = assigner().status;
		Assert.IsFalse(s.Error);
	...
	public IEnumerator AddPushNotificationOnChannels() {
		pn.AddPushNotificationsOnChannels().Channels(channels).Environment(PushEnvironment.Development).Topic("test")
			.Execute(Callback(out var awaiter, out var assigner));
		yield return awaiter;
		var s = assigner().status;
		Assert.IsFalse(s.Error);
	...
	public IEnumerator DeleteMessages() {
		pn.DeleteMessages().Channel(channel).Start(0).End(99999999).Execute(Callback(out var awaiter, out var assigner));
		yield return awaiter;
		var s = assigner().status;
		Assert.IsFalse(s.Error);
	}
	...
	public IEnumerator MessageCounts() {
		pn.MessageCounts().Channels(channels).ChannelsTimetoken(new long[] {0}).Execute(Callback(out var awaiter, out var assigner));
		yield return awaiter;
		var s = assigner().status;
		Assert.IsFalse(s.Error);
	}
	...
	public IEnumerator SetPresenceState() {
		pn.SetPresenceState().Channels(channels).State(new Dictionary<string, object>()).Uuid("test").Execute(Callback(out var awaiter, out var assigner));
		yield return awaiter;
		var s = assigner().status;
		Assert.IsFalse(s.Error);
	}
	...
	public IEnumerator GrantAndRevokeToken() {
		var tokenResources = new Dictionary<string, PNTokenAuthValues>() {{"test", new PNTokenAuthValues() {Get = true}}};
		pn.GrantToken().AuthorizedUuid("test").TTL(10).Resources(new PNTokenResources() {Uuids = tokenResources}).Execute(Callback(out var awaiter, out var assigner));
		yield return awaiter;
		var s = assigner().status;
