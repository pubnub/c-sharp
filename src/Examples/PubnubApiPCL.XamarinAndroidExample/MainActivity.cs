    ...
    public class MainActivity : FragmentActivity
    {
        ...
        public void Subscribe ()
        {
            ...
            ThreadPool.QueueUserWorkItem (o => 
				pubnub.Subscribe<string> (channel, channelGroup, DisplaySubscribeReturnMessage, 
					DisplayConnectDisconnectStatusMessage, DisplayConnectDisconnectStatusMessage, DisplayErrorMessage)
            );
        }
        ...
        public void DetailedHistory ()
        {
            ...
            foreach (string channelToCall in channels) {
                ThreadPool.QueueUserWorkItem (o => 
					pubnub.DetailedHistory (channelToCall.Trim (), 100, false, DisplayDetailedHistoryReturnMessage, DisplayErrorMessage)
                );
            }
        }
        ...
        public void UnsubPresence ()
        {
            ...
            ThreadPool.QueueUserWorkItem (o => 
				pubnub.PresenceUnsubscribe(channel, channelGroup, DisplayConnectDisconnectStatusMessage, 
					DisplayConnectDisconnectStatusMessage, DisplayErrorMessage)
            );
        }
        ...
        void HandleSetValues (object sender, EventArgs ea)
        {
            try {
                ...
                if (cea.cds == CommonDialogStates.Auth) {
                    ...
                    ThreadPool.QueueUserWorkItem (o => 
						pubnub.AuditAccess (cea.channel, cea.valueToSet, DisplayAuditReturnMessage, DisplayErrorMessage)
                    );
                } else if (cea.cds == CommonDialogStates.AuditPresence) {

                    Display ("Running Presence Audit");
                    ThreadPool.QueueUserWorkItem (o => 
						pubnub.AuditPresenceAccess (cea.channel, cea.valueToSet, DisplayAuditReturnMessage, DisplayErrorMessage)
                    );
                } else if (cea.cds == CommonDialogStates.AuditChannelGroupSubscribe) {

                    Display ("Running ChannelGroup Subscribe Audit");
                    ThreadPool.QueueUserWorkItem (o => 
						pubnub.ChannelGroupAuditAccess (cea.channelGroup, cea.valueToSet, DisplayAuditReturnMessage, DisplayErrorMessage)
                    );
                } else if (cea.cds == CommonDialogStates.AuditChannelGroupPresence) {

                    Display ("Running ChannelGroup Presence Audit");
                    ThreadPool.QueueUserWorkItem (o => 
						pubnub.ChannelGroupAuditPresenceAccess (cea.channelGroup, cea.valueToSet, DisplayAuditReturnMessage, DisplayErrorMessage)
                    );

                } else if (cea.cds == CommonDialogStates.RevokePresence) {
                    Display ("Running Presence Revoke");
                    ThreadPool.QueueUserWorkItem (o => 
						pubnub.GrantPresenceAccess (cea.channel, cea.valueToSet, false, false, DisplayGrantReturnMessage, DisplayErrorMessage)
                    );
                    ...
                    ThreadPool.QueueUserWorkItem (o => 
						pubnub.GrantAccess (cea.channel, cea.valueToSet, false, false, DisplayGrantReturnMessage, DisplayErrorMessage)
                    );
                } else if (cea.cds == CommonDialogStates.RevokeChannelGroupPresence) {
                    Display ("Running ChannelGroup Presence Revoke");
                    ThreadPool.QueueUserWorkItem (o => 
						pubnub.ChannelGroupGrantPresenceAccess (cea.channelGroup, cea.valueToSet, false, false, DisplayGrantReturnMessage, DisplayErrorMessage)
                    );
                    ...
                    ThreadPool.QueueUserWorkItem (o => 
						pubnub.ChannelGroupGrantAccess (cea.channelGroup, cea.valueToSet, false, false, DisplayGrantReturnMessage, DisplayErrorMessage)
                    );
                    ...
                    ThreadPool.QueueUserWorkItem (o => 
						pubnub.WhereNow (cea.valueToSet, DisplayWhereNowReturnMessage, DisplayErrorMessage)
                    );
                } else if (cea.cds == CommonDialogStates.GetUserState) {
                    Display ("Running get user state");
                    ThreadPool.QueueUserWorkItem (o => 
						pubnub.GetUserState (cea.channel, cea.channelGroup, cea.valueToSet, DisplayGetUserStateReturnMessage, DisplayErrorMessage)
                    );
                } else if (cea.cds == CommonDialogStates.DeleteUserState) {
                    Display ("Running delete user state");
                    ThreadPool.QueueUserWorkItem (o => 
						pubnub.SetUserState (cea.channel, cea.channelGroup, new KeyValuePair<string, object> (cea.valueToSet, null), DisplaySetUserStateReturnMessage, DisplayErrorMessage)
                    );
                    ...
                    if (Int32.TryParse (cea.valueToSet2, out valueInt)) {
                        ThreadPool.QueueUserWorkItem (o => 
							pubnub.SetUserState (cea.channel, cea.channelGroup, new KeyValuePair<string, object> (cea.valueToSet, valueInt), DisplaySetUserStateReturnMessage, DisplayErrorMessage)
                        );
                    } else if (Double.TryParse (cea.valueToSet2, out valueDouble)) {
                        ThreadPool.QueueUserWorkItem (o => 
							pubnub.SetUserState (cea.channel, cea.channelGroup, new KeyValuePair<string, object> (cea.valueToSet, valueDouble), DisplaySetUserStateReturnMessage, DisplayErrorMessage)
                        );
                    } else {
                        ThreadPool.QueueUserWorkItem (o => 
							pubnub.SetUserState (cea.channel, cea.channelGroup, new KeyValuePair<string, object> (cea.valueToSet, cea.valueToSet2), DisplaySetUserStateReturnMessage, DisplayErrorMessage)
                        );
                    }
                    ...
                    ThreadPool.QueueUserWorkItem (o => 
						pubnub.SetUserState (cea.channel, cea.channelGroup, cea.valueToSet, jsonUserState, DisplaySetUserStateReturnMessage, DisplayErrorMessage)
                    );
                }
            } catch (Exception ex) {
                Display (ex.Message);
        ...
        void HandleAddRemoveFromCgPerms(object sender, EventArgs ea)
        {
            try {
                ...
                if (cea.cds == CommonDialogStates.AddToChannelGroup) {
                    ...
                    ThreadPool.QueueUserWorkItem (o => 
						pubnub.AddChannelsToChannelGroup (new string[] { cea.channel }, cea.channelGroup, DisplayAddChannelToChannelGroupReturnMessage, DisplayErrorMessage)
                    );
                } else if (cea.cds == CommonDialogStates.RemoveFromChannelGroup) {
                    Display ("Running RemoveChannelsFromChannelGroup");
                    ThreadPool.QueueUserWorkItem (o => 
						pubnub.RemoveChannelsFromChannelGroup ( cea.channel.Split(','), cea.channelGroup, DisplayRemoveChannelFromChannelGroupReturnMessage, DisplayErrorMessage)
                    );
                } else if (cea.cds == CommonDialogStates.GetChannelGroup) {
                    Display ("Running GetChannelsForChannelGroup");
                    ThreadPool.QueueUserWorkItem (o => 
						pubnub.GetChannelsForChannelGroup (cea.channelGroup, DisplayGetChannelsForChannelGroupReturnMessage, DisplayErrorMessage)
                    );
                } 
            }
            catch (Exception ex) {
            ...
            try {
                ...
                if (cea.cds == CommonDialogStates.Grant) {
                    if (cea.isPresence) {
                        ...
                        ThreadPool.QueueUserWorkItem (o => 
							pubnub.GrantPresenceAccess (cea.channel, cea.authKey, cea.valToSet2, cea.valToSet1, cea.ttl, DisplayGrantReturnMessage, DisplayErrorMessage)
                        );
                    } else {
                        Display ("Running Subscribe Grant");
                        ThreadPool.QueueUserWorkItem (o => 
							pubnub.GrantAccess (cea.channel, cea.authKey, cea.valToSet2, cea.valToSet1, cea.ttl, DisplayGrantReturnMessage, DisplayErrorMessage)
                        );
                    }
                    ...
                    if (cea.isPresence) {
                        ...
                        ThreadPool.QueueUserWorkItem (o => 
							pubnub.ChannelGroupGrantPresenceAccess (cea.channel, cea.authKey, cea.valToSet2, cea.valToSet1, cea.ttl, DisplayGrantReturnMessage, DisplayErrorMessage)
                        );
                    } else {
                        Display ("Running ChannelGroup Subscribe Grant");
                        ThreadPool.QueueUserWorkItem (o => 
							pubnub.ChannelGroupGrantAccess (cea.channel, cea.authKey, cea.valToSet2, cea.valToSet1, cea.ttl, DisplayGrantReturnMessage, DisplayErrorMessage)
                        );
                    }
                } else if (cea.cds == CommonDialogStates.HereNow) {
                    Display ("Running HereNow");
                    ThreadPool.QueueUserWorkItem (o => 
						pubnub.HereNow (cea.channel.Split(','), cea.valToSet2, cea.valToSet1, DisplayHereNowReturnMessage, DisplayErrorMessage)
                    );
                } else if (cea.cds == CommonDialogStates.GlobalHereNow) {
                    Display ("Running GlobalHereNow");
                    ThreadPool.QueueUserWorkItem (o => 
						pubnub.GlobalHereNow (cea.valToSet2, cea.valToSet1, DisplayGlobalHereNowReturnMessage, DisplayErrorMessage)
                    );
                    ...
                    foreach (string channelToCall in channels) {
                        ThreadPool.QueueUserWorkItem (o => 
							pubnub.Publish (channelToCall.Trim (), mess, cea.valToSet2, "",
								DisplayPublishReturnMessage, DisplayErrorMessage)
                        );
                    }
                }
            } catch (Exception ex) {
		...
		{
			Display (pubnub.JsonPluggableLibrary.SerializeToJsonString(result));
		}
		...
		{
			Display ("Total Records = " + result.Message.Length.ToString());
			for (int index = 0; index < result.Message.Length; index++)
			{
				Display (pubnub.JsonPluggableLibrary.SerializeToJsonString(result.Message[index]));
			}
