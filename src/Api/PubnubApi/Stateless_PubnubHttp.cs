...
namespace PubnubApi
{
    public class Stateless_PubnubHttp : IStatelessPubnubHttp
    {
...
#if !NET35 && !NET40 && !NET45 && !NET461 && !NETSTANDARD10
        public Stateless_PubnubHttp(PNConfiguration config, IJsonPluggableLibrary jsonPluggableLibrary, IPubnubLog log, EndPoint.TelemetryManager telemetryManager, HttpClient refHttpClientSubscribe, HttpClient refHttpClientNonsubscribe)
#else
        public Stateless_PubnubHttp(PNConfiguration config, IJsonPluggableLibrary jsonPluggableLibrary, IPubnubLog log, EndPoint.TelemetryManager telemetryManager)
#endif
        {
            pubnubConfig = config;
        ...
        HttpWebRequest IStatelessPubnubHttp.SetProxy<T>(HttpWebRequest request)
        {
#if !NETSTANDARD10
        ...
        HttpWebRequest IStatelessPubnubHttp.SetTimeout<T>(RequestState<T> pubnubRequestState, HttpWebRequest request)
        {
        ...
        HttpWebRequest IStatelessPubnubHttp.SetNoCache<T>(HttpWebRequest request)
        {
        ...
        HttpWebRequest IStatelessPubnubHttp.SetServicePointSetTcpKeepAlive(HttpWebRequest request)
        {
            //do nothing
            return request;
        }

        async Task<string> IStatelessPubnubHttp.SendRequestAndGetJsonResponse<T>(Uri requestUri, RequestState<T> pubnubRequestState, HttpWebRequest request, CancellationToken cancellationToken)
        {
            if (pubnubConfig.UseClassicHttpWebRequest)
            {
                return await SendRequestAndGetJsonResponseClassicHttp(requestUri, pubnubRequestState, request, cancellationToken).ConfigureAwait(false);
            }
            else
            {
#if !NET35 && !NET40 && !NET45 && !NET461 && !NETSTANDARD10
                ...
                {
                    return await SendRequestAndGetJsonResponseTaskFactory(pubnubRequestState, request, cancellationToken).ConfigureAwait(false);
                }
                else
                {
                    return await SendRequestAndGetJsonResponseHttpClient(requestUri, pubnubRequestState, request, cancellationToken).ConfigureAwait(false);
                }
#else
                return await SendRequestAndGetJsonResponseTaskFactory(pubnubRequestState, request, cancellationToken).ConfigureAwait(false);
#endif
        ...
        async Task<byte[]> IStatelessPubnubHttp.SendRequestAndGetStreamResponse<T>(Uri requestUri, RequestState<T> pubnubRequestState, HttpWebRequest request, CancellationToken cancellationToken)
        {
            if (pubnubConfig.UseClassicHttpWebRequest)
            {
                return await SendRequestAndGetStreamResponseClassicHttp(pubnubRequestState, request, cancellationToken).ConfigureAwait(false);
            }
            else
            {
#if !NET35 && !NET40 && !NET45 && !NET461 && !NETSTANDARD10
                if (pubnubConfig.UseTaskFactoryAsyncInsteadOfHttpClient)
                {
                    return await SendRequestAndGetStreamResponseTaskFactory(pubnubRequestState, request, cancellationToken).ConfigureAwait(false);
                }
                else
                {
                    return await SendRequestAndGetStreamResponseHttpClient(requestUri, pubnubRequestState, cancellationToken).ConfigureAwait(false);
                }
#else
                return await SendRequestAndGetStreamResponseTaskFactory(pubnubRequestState, request, cancellationToken).ConfigureAwait(false);
#endif
        ...
        async Task<string> IStatelessPubnubHttp.SendRequestAndGetJsonResponseWithPOST<T>(Uri requestUri, RequestState<T> pubnubRequestState, HttpWebRequest request, byte[] postData, string contentType, CancellationToken cancellationToken)
        {
            ...
            if (pubnubConfig.UseClassicHttpWebRequest)
            {
                return await SendRequestAndGetJsonResponseClassicHttpWithPOST(pubnubRequestState, request, postData, contentType, cancellationToken).ConfigureAwait(false);
            }
            else
            {
#if !NET35 && !NET40 && !NET45 && !NET461 && !NETSTANDARD10
                if (pubnubConfig.UseTaskFactoryAsyncInsteadOfHttpClient)
                {
                    return await SendRequestAndGetJsonResponseTaskFactoryWithPOST(pubnubRequestState, request, postData, contentType, cancellationToken).ConfigureAwait(false);
                }
                else
                {
                    return await SendRequestAndGetJsonResponseHttpClientWithPOST(requestUri, pubnubRequestState, postData, contentType, cancellationToken).ConfigureAwait(false);
                }
#else
                return await SendRequestAndGetJsonResponseTaskFactoryWithPOST(pubnubRequestState, request, postData, contentType, cancellationToken).ConfigureAwait(false);
#endif
        ...
        async Task<string> IStatelessPubnubHttp.SendRequestAndGetJsonResponseWithPATCH<T>(Uri requestUri, RequestState<T> pubnubRequestState, HttpWebRequest request, byte[] patchData, CancellationToken cancellationToken)
        {
            ...
            if (pubnubConfig.UseClassicHttpWebRequest)
            {
                return await SendRequestAndGetJsonResponseClassicHttpWithPATCH(pubnubRequestState, request, patchData, cancellationToken).ConfigureAwait(false);
            }
            else
            {
#if !NET35 && !NET40 && !NET45 && !NET461 && !NETSTANDARD10
                if (pubnubConfig.UseTaskFactoryAsyncInsteadOfHttpClient)
                {
                    return await SendRequestAndGetJsonResponseTaskFactoryWithPATCH(pubnubRequestState, request, patchData, cancellationToken).ConfigureAwait(false);
                }
                else
                {
                    return await SendRequestAndGetJsonResponseHttpClientWithPATCH(requestUri, pubnubRequestState, patchData, cancellationToken).ConfigureAwait(false);
                }
#else
                return await SendRequestAndGetJsonResponseTaskFactoryWithPATCH(pubnubRequestState, request, patchData, cancellationToken).ConfigureAwait(false);
#endif
...
#if !NET35 && !NET40 && !NET45 && !NET461 && !NETSTANDARD10
        async Task<string> SendRequestAndGetJsonResponseHttpClient<T>(Uri requestUri, RequestState<T> pubnubRequestState, HttpWebRequest request, CancellationToken cancellationToken)
        {
            string jsonString = "";
        ...
        async Task<byte[]> SendRequestAndGetStreamResponseHttpClient<T>(Uri requestUri, RequestState<T> pubnubRequestState, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                System.Diagnostics.Stopwatch stopWatch = new System.Diagnostics.Stopwatch();
                stopWatch.Start();
                response = await httpClientNonsubscribe.GetAsync(requestUri, cancellationToken).ConfigureAwait(false);
                if (response.IsSuccessStatusCode || response.Content != null)
                {
                    var stream = await response.Content.ReadAsStreamAsync().ConfigureAwait(false);
                    stopWatch.Stop();
        ...
        async Task<string> SendRequestAndGetJsonResponseHttpClientWithPOST<T>(Uri requestUri, RequestState<T> pubnubRequestState, byte[] postData, string contentType, CancellationToken cancellationToken)
        {
            string jsonString = "";
            HttpResponseMessage response = null;
            //CancellationTokenSource cts = new CancellationTokenSource();
            try
            {
                ...
                if (pubnubRequestState.ResponseType == PNOperationType.PNSubscribeOperation)
                {
                    response = await httpClientSubscribe.PostAsync(requestUri, postDataContent, cancellationToken).ConfigureAwait(false);
                }
                else
                {
                    response = await httpClientNonsubscribe.PostAsync(requestUri, postDataContent, cancellationToken).ConfigureAwait(false);
                }
        ...
        async Task<string> SendRequestAndGetJsonResponseHttpClientWithPATCH<T>(Uri requestUri, RequestState<T> pubnubRequestState, byte[] patchData, CancellationToken cancellationToken)
        {
            string jsonString = "";
            HttpResponseMessage response = null;
            //CancellationTokenSource cts = new CancellationTokenSource();
            try
            {
                ...
                if (pubnubRequestState.ResponseType == PNOperationType.PNSubscribeOperation)
                {
                    response = await httpClientSubscribe.SendAsync(requestMsg, cancellationToken).ConfigureAwait(false);
                }
                else
                {
                    response = await httpClientNonsubscribe.SendAsync(requestMsg, cancellationToken).ConfigureAwait(false);
                }
        ...
        async Task<string> SendRequestAndGetJsonResponseTaskFactory<T>(RequestState<T> pubnubRequestState, HttpWebRequest request, CancellationToken cancellationToken)
        {
        ...
        async Task<byte[]> SendRequestAndGetStreamResponseTaskFactory<T>(RequestState<T> pubnubRequestState, HttpWebRequest request, CancellationToken cancellationToken)
        {
            HttpWebResponse response = null;
            byte[] streamBytes;
        ...
        async Task<string> SendRequestAndGetJsonResponseTaskFactoryWithPOST<T>(RequestState<T> pubnubRequestState, HttpWebRequest request, byte[] postData, string contentType, CancellationToken cancellationToken)
        {
        ...
        async Task<string> SendRequestAndGetJsonResponseTaskFactoryWithPATCH<T>(RequestState<T> pubnubRequestState, HttpWebRequest request, byte[] patchData, CancellationToken cancellationToken)
        {
        ...
        async Task<string> SendRequestAndGetJsonResponseClassicHttp<T>(Uri requestUri, RequestState<T> pubnubRequestState, HttpWebRequest request, CancellationToken cancellationToken)
        {
        ...
        async Task<byte[]> SendRequestAndGetStreamResponseClassicHttp<T>(RequestState<T> pubnubRequestState, HttpWebRequest request, CancellationToken cancellationToken)
        {
        ...
        async Task<string> SendRequestAndGetJsonResponseClassicHttpWithPOST<T>(RequestState<T> pubnubRequestState, HttpWebRequest request, byte[] postData, string contentType, CancellationToken cancellationToken)
        {
        ...
        async Task<string> SendRequestAndGetJsonResponseClassicHttpWithPATCH<T>(RequestState<T> pubnubRequestState, HttpWebRequest request, byte[] patchData, CancellationToken cancellationToken)
        {
