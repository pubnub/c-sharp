...
namespace PubnubApi.EndPoint
{
    public class PublishOperation : PubnubCoreBase
    {
        ...
        public PublishOperation(PNConfiguration pubnubConfig, IJsonPluggableLibrary jsonPluggableLibrary,
        ...
        public PNPublishResult Sync()
        {
            ...
            Task<PNPublishResult> task = Task<PNPublishResult>.Factory.StartNew(() =>
            {
                ...
                Publish(channelName, publishContent, storeInHistory, ttl, userMetadata, queryParam,
                    new PNPublishResultExt((r, s) =>
                    {
                        SyncResult = r;
                        syncEvent.Set();
                    }));
        ...
        internal void Publish(string channel, object message, bool storeInHistory, int ttl,
            Dictionary<string, object> userMetadata, Dictionary<string, object> externalQueryParam,
            PNCallback<PNPublishResult> callback)
        {
            if (string.IsNullOrEmpty(channel) || string.IsNullOrEmpty(channel.Trim()) || message == null)
            {
                PNStatus status = new PNStatus();
                status.Error = true;
                status.ErrorData = new PNErrorData("Missing Channel or Message",
                    new ArgumentException("Missing Channel or Message"));
                callback.OnResponse(null, status);
                return;
            }

            if (string.IsNullOrEmpty(config.PublishKey) || string.IsNullOrEmpty(config.PublishKey.Trim()) ||
                config.PublishKey.Length <= 0)
            {
                PNStatus status = new PNStatus();
                status.Error = true;
                status.ErrorData = new PNErrorData("Invalid publish key",
                    new MissingMemberException("Invalid publish key"));
                callback.OnResponse(null, status);
                return;
            }
            ...
            var transportRequest = PubnubInstance.transportMiddleware.PreapareTransportRequest(
                requestParameter: requestParameters, operationType: PNOperationType.PNPublishOperation);
            ...
            PubnubInstance.transportMiddleware.Send(transportRequest).ContinueWith(t =>
            {
                ...
                else
                {
                    ...
                    PNStatus status = new StatusBuilder(config, jsonLibrary).CreateStatusResponse(
                        PNOperationType.PNPublishOperation, category, requestState, statusCode,
        ...
        internal async Task<PNResult<PNPublishResult>> Publish(string channel, object message, bool storeInHistory,
            int ttl, Dictionary<string, object> metaData, Dictionary<string, object> externalQueryParam)
        {
            ...
            if (string.IsNullOrEmpty(channel) || string.IsNullOrEmpty(channel.Trim()) || message == null)
            {
                ...
                {
                    Error = true,
                    ErrorData = new PNErrorData("Missing Channel or Message",
                        new ArgumentException("Missing Channel or Message"))
                };
                ...
                config.PublishKey.Length <= 0)
            {
                ...
                errStatus.ErrorData = new PNErrorData("Invalid publish key",
                    new MissingMemberException("Invalid publish key"));
                returnValue.Status = errStatus;
                return returnValue;
            }
        ...
        private string PrepareContent(object originalMessage)
        {
            ...
            {
                config.CryptoModule ??=
                    new CryptoModule(new LegacyCryptor(config.CipherKey, config.UseRandomInitializationVector), null);
        ...
        private RequestParameter CreateRequestParameter()
        {
            ...
            if (userMetadata != null)
            {
                ...
                requestQueryStringParams.Add("meta",
                    UriUtil.EncodeUriComponent(jsonMetaData, PNOperationType.PNPublishOperation, false, false, false));
            }
                ...
                foreach (KeyValuePair<string, object> kvp in queryParam)
                {
                    if (!requestQueryStringParams.ContainsKey(kvp.Key))
                    {
                        requestQueryStringParams.Add(kvp.Key,
                            UriUtil.EncodeUriComponent(kvp.Value.ToString(), PNOperationType.PNPublishOperation, false,
                                false, false));
                    }
                }
            }
