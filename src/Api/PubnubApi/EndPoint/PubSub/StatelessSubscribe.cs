...
namespace PubnubApi.EndPoint
{
  public class StatelessSubscribeOperation<T> //: Stateless_PubnubCoreBase
  {
    ...
    public int reconnectionElpasedCount;
  ...
  public class ReconnectingState<T> : SubscribeState<T>
  {
    ...
    public async override Task<CancellationToken> Request()
    {
      ...
      switch (currentTrigger)
      {
        case SubscribeEvent.Reconnect:
        case SubscribeEvent.Fail:
          await InvokeReconnectingStateEffects(receiveTokenSource.Token);
          break;
        case SubscribeEvent.Giveup:
          break;
        default:
    ...
    private async Task<bool> InvokeReconnectingStateEffects(CancellationToken cancellationToken)
    {
      bool ret = false;
      System.Diagnostics.Debug.WriteLine("InvokeReconnectingStateEffects");
      if (this.subscribeOperation.reconnectionElpasedCount < 5)   // TODO: Fetch from configuration
      {
        var delayInterval = (int)(Math.Pow(2, ++this.subscribeOperation.reconnectionElpasedCount) - 1); // TODO: calculate accroding to retry policy
        await Task.Delay(delayInterval * 1000);

        StatelessSubscribeManager subscribeManager = new StatelessSubscribeManager(subscribeOperation.Config, subscribeOperation.JsonLibrary, subscribeOperation.Unit, subscribeOperation.PubnubLog, subscribeOperation.PubnubTelemetryMgr, subscribeOperation.PubnubTokenMgr);
        Tuple<string, PNStatus> attemptResponse = this.subscribeOperation.State.SubscribeData.Timetoken == 0 ?
        await subscribeManager.Handshake<string>(PNOperationType.PNSubscribeOperation, (Channels != null) ? Channels.ToArray() : null, (ChannelGroups != null) ? ChannelGroups.ToArray() : null, null, QueryParam, cancellationToken) :
        await subscribeManager.ReceiveMessages<T>(PNOperationType.PNSubscribeOperation, (Channels != null) ? Channels.ToArray() : null, (ChannelGroups != null) ? ChannelGroups.ToArray() : null, timetoken, SubscribeData.Region, false, null, null, cancellationToken);


        PNStatus status = attemptResponse.Item2;
        if (!status.Error && status.StatusCode == 200)
        {

          System.Diagnostics.Debug.WriteLine($"Reconnection success. {attemptResponse.Item1}");

          List<object> receiveEventResult = null;
          string jsonString = attemptResponse.Item1;
          object deserializedResult = subscribeOperation.JsonLibrary.DeserializeToObject(jsonString);
          List<object> result1 = ((IEnumerable)deserializedResult).Cast<object>().ToList();

          if (result1 != null && result1.Count > 0)
          {
            receiveEventResult = result1;
          }

          long jsonTimetoken = GetTimetokenFromMultiplexResult(receiveEventResult);
          int jsonRegion = GetRegionFromMultiplexResult(receiveEventResult);

          SubscribeData cursorData = new SubscribeData();
          cursorData.Timetoken = jsonTimetoken;
          cursorData.Region = jsonRegion;

          // TODO: Handling messages delivery. If this attempt is with non zero timetoken then, Response may contain message part
          this.subscribeOperation.reconnectionElpasedCount = 0;
          ret = true;

          SubscribeState<T> nextState = ChangeSubscribeState(SubscribeOperation, SubscribeEvent.Success);
          nextState.SubscribeData = cursorData;
          await nextState.Request();
        }
        else
        {
          // Attempt Failed
          System.Diagnostics.Debug.WriteLine($"Reconnection attempt ${this.subscribeOperation.reconnectionElpasedCount}... Failed");
      ...
      {
        System.Diagnostics.Debug.WriteLine($"Reconnection attempts exhaused... Giving Up");

        SubscribeState<T> nextState = ChangeSubscribeState(SubscribeOperation, SubscribeEvent.Giveup);
        await nextState.Request();
      }
      return ret;
    }
