...
namespace PubnubApi.EndPoint
{
    public class SignalOperation : PubnubCoreBase
    {
        ...
        public void Execute(PNCallback<PNPublishResult> callback)
        {
            ...
            if (string.IsNullOrEmpty(this.channelName) || string.IsNullOrEmpty(channelName.Trim()) ||
        ...
        private void Signal(string channel, object message, Dictionary<string, object> metaData,
            Dictionary<string, object> externalQueryParam, PNCallback<PNPublishResult> callback)
        {
                ...
                config.PublishKey.Length <= 0)
            {
                ...
                {
                    Error = true,
                    ErrorData = new PNErrorData("Invalid publish key", new ArgumentException("Invalid publish key"))
                };
                callback.OnResponse(null, status);
                return;
            }
            ...
            RequestState<PNPublishResult> requestState = new RequestState<PNPublishResult>
            {
                ...
                ResponseType = PNOperationType.PNSignalOperation,
            ...
            var transportRequest =
                PubnubInstance.transportMiddleware.PreapareTransportRequest(requestParameter: requestParameters,
                    operationType: PNOperationType.PNSignalOperation);
            PubnubInstance.transportMiddleware.Send(transportRequest).ContinueWith(t =>
            {
                            ...
                            else
                            {
                                ...
                                PNStatus status =
                                    new StatusBuilder(config, jsonLibrary).CreateStatusResponse<PNPublishResult>(
                                        PNOperationType.PNSignalOperation, category, requestState, 400,
                ...
                else
                {
                    ...
                    PNStatusCategory category =
                        PNStatusCategoryHelper.GetPNStatusCategory(statusCode, t.Result.Error.Message);
                    PNStatus status = new StatusBuilder(config, jsonLibrary).CreateStatusResponse(
                        PNOperationType.PNSignalOperation, category, requestState, statusCode,
                        new PNException(t.Result.Error.Message, t.Result.Error));
                    requestState.PubnubCallback.OnResponse(default, status);
                    logger?.Info(
                        $"{GetType().Name} request finished with status code {requestState.Response?.StatusCode}");
                }
        ...
        private async Task<PNResult<PNPublishResult>> Signal(string channel, object message,
            Dictionary<string, object> metaData, Dictionary<string, object> externalQueryParam)
        {
