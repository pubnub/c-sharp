...
{
    public class GetEntitiesOperation : PubnubCoreBase
    {
        private readonly PNConfiguration config;
        private readonly IJsonPluggableLibrary jsonLibrary;
        private readonly IPubnubUnitTest unit;
        private readonly GetEntitiesParameters parameters;

        private PNCallback<PNDataSyncEntitiesListResult> savedCallback;

        private const PNOperationType OperationType = PNOperationType.PNDataSyncGetEntities;

        public GetEntitiesOperation(PNConfiguration pubnubConfig, IJsonPluggableLibrary jsonPluggableLibrary,
            IPubnubUnitTest pubnubUnit, TokenManager tokenManager, Pubnub instance,
            GetEntitiesParameters parameters) : base(pubnubConfig,
        ...
        public void Execute(PNCallback<PNDataSyncEntitiesListResult> callback)
        {
            if (callback == null)
            {
        ...
        public async Task<PNResult<PNDataSyncEntitiesListResult>> ExecuteAsync()
        {
            logger?.Trace($"{GetType().Name} ExecuteAsync invoked.");
            return await GetEntitiesAsync().ConfigureAwait(false);
        }
        ...
        private async Task<PNResult<PNDataSyncEntitiesListResult>> GetEntitiesAsync()
        {
            var returnValue = new PNResult<PNDataSyncEntitiesListResult>();
            ...
            var requestState = new RequestState<PNDataSyncEntitiesListResult>
            {
            ...
            if (transportResponse.Error == null)
            {
                ...
                if (!string.IsNullOrEmpty(json))
                {
                    ...
                    var responseResult =
                        responseBuilder.JsonToObject<PNDataSyncEntitiesListResult>(resultList, true);
