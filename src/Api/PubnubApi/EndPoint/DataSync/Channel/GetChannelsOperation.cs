...
{
    public class GetChannelsOperation : PubnubCoreBase
    {
        private readonly PNConfiguration config;
        private readonly IJsonPluggableLibrary jsonLibrary;
        private readonly IPubnubUnitTest unit;
        private readonly GetChannelsParameters parameters;

        private PNCallback<PNDataSyncChannelsListResult> savedCallback;

        private const PNOperationType OperationType = PNOperationType.PNDataSyncGetChannels;

        public GetChannelsOperation(PNConfiguration pubnubConfig, IJsonPluggableLibrary jsonPluggableLibrary,
            IPubnubUnitTest pubnubUnit, TokenManager tokenManager, Pubnub instance,
            GetChannelsParameters parameters) : base(pubnubConfig,
        ...
        public void Execute(PNCallback<PNDataSyncChannelsListResult> callback)
        {
            if (callback == null)
            {
        ...
        public async Task<PNResult<PNDataSyncChannelsListResult>> ExecuteAsync()
        {
            logger?.Trace($"{GetType().Name} ExecuteAsync invoked.");
            return await GetChannelsAsync().ConfigureAwait(false);
        }
        ...
        private async Task<PNResult<PNDataSyncChannelsListResult>> GetChannelsAsync()
        {
            var returnValue = new PNResult<PNDataSyncChannelsListResult>();
            ...
            var requestState = new RequestState<PNDataSyncChannelsListResult>
            {
            ...
            if (transportResponse.Error == null)
            {
                ...
                if (!string.IsNullOrEmpty(json))
                {
                    ...
                    var responseResult =
                        responseBuilder.JsonToObject<PNDataSyncChannelsListResult>(resultList, true);
