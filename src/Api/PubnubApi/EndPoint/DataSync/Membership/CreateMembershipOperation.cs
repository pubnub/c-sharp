...
{
    public class CreateMembershipOperation : PubnubCoreBase
    {
        private readonly PNConfiguration config;
        private readonly IJsonPluggableLibrary jsonLibrary;
        private readonly IPubnubUnitTest unit;
        private readonly CreateMembershipParameters parameters;

        private PNCallback<PNDataSyncMembershipResult> savedCallback;
        
        private const PNOperationType OperationType = PNOperationType.PNDataSyncCreateMembership;

        public CreateMembershipOperation(PNConfiguration pubnubConfig, IJsonPluggableLibrary jsonPluggableLibrary,
            IPubnubUnitTest pubnubUnit, TokenManager tokenManager, Pubnub instance,
            CreateMembershipParameters parameters) : base(pubnubConfig,
        ...
        public async Task<PNResult<PNDataSyncMembershipResult>> ExecuteAsync()
        {
            ...
            return await CreateMembershipAsync().ConfigureAwait(false);
        }
        ...
        private async Task<PNResult<PNDataSyncMembershipResult>> CreateMembershipAsync()
        {
            var returnValue = new PNResult<PNDataSyncMembershipResult>();

            if (string.IsNullOrEmpty(parameters.ChannelId) || string.IsNullOrEmpty(parameters.ChannelId.Trim()))
            {
                var errStatus = new PNStatus
                {
                    Error = true,
                    ErrorData = new PNErrorData("Missing ChannelId",
                        new ArgumentException("Missing ChannelId"))
                };
                returnValue.Status = errStatus;
                return returnValue;
            }

            if (string.IsNullOrEmpty(parameters.UserId) || string.IsNullOrEmpty(parameters.UserId.Trim()))
            {
                var errStatus = new PNStatus
                {
                    Error = true,
                    ErrorData = new PNErrorData("Missing UserId",
