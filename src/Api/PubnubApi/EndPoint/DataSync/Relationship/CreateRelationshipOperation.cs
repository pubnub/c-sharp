...
{
    public class CreateRelationshipOperation : PubnubCoreBase
    {
        private readonly PNConfiguration config;
        private readonly IJsonPluggableLibrary jsonLibrary;
        private readonly IPubnubUnitTest unit;
        private readonly CreateRelationshipParameters parameters;

        private PNCallback<PNDataSyncRelationshipResult> savedCallback;
        
        private const PNOperationType OperationType = PNOperationType.PNDataSyncCreateRelationship;

        public CreateRelationshipOperation(PNConfiguration pubnubConfig, IJsonPluggableLibrary jsonPluggableLibrary,
            IPubnubUnitTest pubnubUnit, TokenManager tokenManager, Pubnub instance,
            CreateRelationshipParameters parameters) : base(pubnubConfig,
        ...
        public async Task<PNResult<PNDataSyncRelationshipResult>> ExecuteAsync()
        {
            ...
            return await CreateRelationshipAsync().ConfigureAwait(false);
        }
        ...
        private async Task<PNResult<PNDataSyncRelationshipResult>> CreateRelationshipAsync()
        {
            var returnValue = new PNResult<PNDataSyncRelationshipResult>();

            if (string.IsNullOrEmpty(parameters.EntityAId) || string.IsNullOrEmpty(parameters.EntityAId.Trim()))
            {
                var errStatus = new PNStatus
                {
                    Error = true,
                    ErrorData = new PNErrorData("Missing EntityAId",
                        new ArgumentException("Missing EntityAId"))
                };
                returnValue.Status = errStatus;
                return returnValue;
            }

            if (string.IsNullOrEmpty(parameters.EntityBId) || string.IsNullOrEmpty(parameters.EntityBId.Trim()))
            {
                var errStatus = new PNStatus
                {
                    Error = true,
                    ErrorData = new PNErrorData("Missing EntityBId",
                        new ArgumentException("Missing EntityBId"))
                };
                returnValue.Status = errStatus;
                return returnValue;
            }
        ...
        private RequestParameter CreateRequestParameter()
        {
            ...
            dataProperties.Add("entityAId", parameters.EntityAId);
            dataProperties.Add("entityBId", parameters.EntityBId);
            dataProperties.Add("relationshipClass", parameters.RelationshipClass);
