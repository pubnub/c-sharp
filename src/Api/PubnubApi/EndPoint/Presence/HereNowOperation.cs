...
namespace PubnubApi.EndPoint
{
    public class HereNowOperation : PubnubCoreBase
    {
        ...
        private void HereNow(PNCallback<PNHereNowResult> callback)
        {
            ...
            var transportRequest =
                PubnubInstance.transportMiddleware.PreapareTransportRequest(requestParameter: requestParameter,
                    operationType: PNOperationType.PNHereNowOperation);
            PubnubInstance.transportMiddleware.Send(transportRequest: transportRequest).ContinueWith(t =>
            {
                ...
                else
                {
                    ...
                    PNStatus status = new StatusBuilder(config, jsonLibrary).CreateStatusResponse(
                        PNOperationType.PNHereNowOperation, category, requestState, statusCode,
        ...
        {
            string channel = (channelNames != null && channelNames.Length > 0)
                ? string.Join(",", channelNames.OrderBy(x => x).ToArray())
                : ",";
            ...
            string commaDelimitedchannelGroup = (channelGroupNames != null)
                ? string.Join(",", channelGroupNames.OrderBy(x => x).ToArray())
                : "";
            if (!string.IsNullOrEmpty(commaDelimitedchannelGroup) && commaDelimitedchannelGroup.Trim().Length > 0)
            {
                requestQueryStringParams.Add("channel-group",
                    UriUtil.EncodeUriComponent(commaDelimitedchannelGroup, PNOperationType.PNHereNowOperation, false,
                        false, false));
            }
                ...
                foreach (KeyValuePair<string, object> kvp in queryParam)
                {
                    if (!requestQueryStringParams.ContainsKey(kvp.Key))
                    {
                        requestQueryStringParams.Add(kvp.Key,
                            UriUtil.EncodeUriComponent(kvp.Value.ToString(), PNOperationType.PNHereNowOperation, false,
                                false, false));
                    }
                }
            }
