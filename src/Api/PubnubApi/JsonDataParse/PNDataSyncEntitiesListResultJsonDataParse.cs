...
internal static class PNDataSyncEntitiesListResultJsonDataParse
{
    internal static PNDataSyncEntitiesListResult GetObject(IJsonPluggableLibrary jsonPlug, List<object> listObject)
    {
        var result = new PNDataSyncEntitiesListResult();
        foreach (var rawObject in listObject)
        {
            ...
            if (objectDictionary.TryGetValue("data", out var dataObject) && dataObject != null)
            {
                ...
                for (int i = 0; i < dataArray.Length; i++)
                {
                    ...
                    var data = new PNDataSyncEntityResult
                    {
