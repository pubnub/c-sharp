...
internal static class PNDataSyncChannelsListResultJsonDataParse
{
    internal static PNDataSyncChannelsListResult GetObject(IJsonPluggableLibrary jsonPlug, List<object> listObject)
    {
        var result = new PNDataSyncChannelsListResult();
        foreach (var rawObject in listObject)
        {
            ...
            if (objectDictionary.TryGetValue("data", out var dataObject) && dataObject != null)
            {
                ...
                for (int i = 0; i < dataArray.Length; i++)
                {
                    ...
                    var data = new PNDataSyncChannelResult
                    {
