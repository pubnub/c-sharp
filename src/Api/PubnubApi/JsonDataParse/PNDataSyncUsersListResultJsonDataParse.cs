...
internal static class PNDataSyncUsersListResultJsonDataParse
{
    internal static PNDataSyncUsersListResult GetObject(IJsonPluggableLibrary jsonPlug, List<object> listObject)
    {
        var result = new PNDataSyncUsersListResult();
        foreach (var rawObject in listObject)
        {
            ...
            if (objectDictionary.TryGetValue("data", out var dataObject) && dataObject != null)
            {
                ...
                for (int i = 0; i < dataArray.Length; i++)
                {
                    ...
                    var data = new PNDataSyncUserResult
                    {
