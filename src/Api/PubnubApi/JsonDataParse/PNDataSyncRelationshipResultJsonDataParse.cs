...
internal static class PNDataSyncRelationshipResultJsonDataParse
{
    internal static PNDataSyncRelationshipResult GetObject(IJsonPluggableLibrary jsonPlug, List<object> listObject)
    {
        Dictionary<string, object> dictionaryObject = (listObject != null && listObject.Count == 1)
            ...
            : null;
        PNDataSyncRelationshipResult result = null;
        if (dictionaryObject != null && dictionaryObject.ContainsKey("data"))
        {
            result = new PNDataSyncRelationshipResult();
            ...
            if (dataDictionary != null && dataDictionary.Count > 0)
            {
                result.Id = dataDictionary.ContainsKey("id") && dataDictionary["id"] != null
                    ...
                    : null;
                result.EntityAId = dataDictionary.ContainsKey("entityAId") && dataDictionary["entityAId"] != null
                    ? dataDictionary["entityAId"].ToString()
                    : null;
                result.EntityBId = dataDictionary.ContainsKey("entityBId") && dataDictionary["entityBId"] != null
