...
public class PNDataSyncRelationshipsListResultJsonDataParse
{
    ...
    {
        var result = new PNDataSyncRelationshipsListResult();
        foreach (var rawObject in listObject)
        {
            ...
            if (objectDictionary.TryGetValue("data", out var dataObject) && dataObject != null)
            {
                ...
                for (int i = 0; i < dataArray.Length; i++)
                {
                    ...
                    var data = new PNDataSyncRelationshipResult
                    {
                        Id = dataEntryDictionary.ContainsKey("id") && dataEntryDictionary["id"] != null
                            ? dataEntryDictionary["id"].ToString()
                            : null,
                        EntityAId = dataEntryDictionary.ContainsKey("entityAId") && dataEntryDictionary["entityAId"] != null
                            ? dataEntryDictionary["entityAId"].ToString()
                            : null,
                        EntityBId = dataEntryDictionary.ContainsKey("entityBId") && dataEntryDictionary["entityBId"] != null
