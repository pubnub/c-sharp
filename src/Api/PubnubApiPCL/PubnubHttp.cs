...
namespace PubnubApi
{
    public class PubnubHttp : IPubnubHttp
    {
        ...
        async Task<string> IPubnubHttp.SendRequestAndGetJsonResponse<T>(Uri requestUri, RequestState<T> pubnubRequestState, PubnubWebRequest request)
        {
            ...
            PubnubWebResponse response = await Task.Factory.FromAsync<PubnubWebResponse>(request.BeginGetResponse, asyncPubnubResult => (PubnubWebResponse)request.EndGetResponse(asyncPubnubResult), pubnubRequestState);
