...
namespace AcceptanceTests.Steps
{
    ...
    public class SendAFileToSpaceSteps
    {
        ...
        private PNFileUploadResult fileUploadResult = null;
        ...
        public SendAFileToSpaceSteps(ScenarioContext scenarioContext) 
        { 
        ...
        [When(@"I send a file with '([^']*)' space id and '([^']*)' message type")]
        public async Task WhenISendAFileWithSpaceIdAndMessageType(string p0, string p1)
        {
            string targetFileUpload = @"fileupload.txt";
            PNResult<PNFileUploadResult> fileUploadResultResp = await pn.SendFile()
                .Channel("my_channel")
            ...
            fileUploadResult = fileUploadResultResp.Result;
            pnStatus = fileUploadResultResp.Status;
            if (pnStatus.Error)
            {
