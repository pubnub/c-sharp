...
namespace AcceptanceTests.Steps
{
    ...
    public class FeatureFetchHistoryForVSPSteps
    {
        ...
        public void ThenHistoryResponseContainsMessagesWithAndTypes(string message, string file)
        {
            bool gotExpectedResult = false;
            if (!pnStatus.Error && fetchHistoryResult != null && fetchHistoryResult.Messages != null)
            {
        ...
        public async Task WhenIFetchMessageHistoryWithSetToForChannel(string includeFieldName, string @false, string p2)
        {
            bool includeType = true;
            bool includeSpaceId = false;
            PNResult<PNFetchHistoryResult> fetchHistoryResponse = null;
            if (string.Compare("includeType", includeFieldName, true) == 0)
            {
                includeType = Boolean.Parse(@false);
                fetchHistoryResponse = await pn.FetchHistory()
                    .Channels(new string[]{ p2 })
                    .IncludeType(includeType)
                    .ExecuteAsync();
            }
            else if (string.Compare("includeSpaceId", includeFieldName, true) == 0)
            {
        ...
        public void ThenHistoryResponseContainsMessagesWithoutTypes()
        {
            bool gotExpectedResult = false;
            if (!pnStatus.Error && fetchHistoryResult != null && fetchHistoryResult.Messages != null)
            {
