...
            "
        ...
        [Test]
        public static void ThenPublishWithLargePayloadShouldUsePostAutomatically()
        {
            server.ClearRequests();
            ...
            // For this large payload, expect a POST request without the message in the path
            server.AddRequest(new Request()
                    .WithMethod("POST")
            ...
            pubnub.PubnubUnitTest = null;
            pubnub = null;
            Assert.IsTrue(receivedPublishMessage, "Publish with large payload Failed");
        }
