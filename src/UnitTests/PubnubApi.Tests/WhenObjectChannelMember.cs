...
namespace PubNubMessaging.Tests
{
    ...
    public class WhenObjectChannelMember : TestHarness
    {
        ...
        [Test]
        public static void ThenSetRemoveUuidMetadataShouldReturnSuccessCodeAndInfo()
        {
            server.ClearRequests();

            if (PubnubCommon.EnableStubTest)
            {
                Assert.Ignore("Ignored ThenSetRemoveUuidMetadataShouldReturnSuccessCodeAndInfo");
                return;
            }
            ...
            if (!receivedMessage)
            {
                Assert.IsTrue(receivedMessage, "Set/Remove UuidMetadata Failed");
            }
        ...
        [Test]
#if NET40
        public static void ThenWithAsyncSetRemoveUuidMetadataShouldReturnSuccessCodeAndInfo()
#else
        public static async Task ThenWithAsyncSetRemoveUuidMetadataShouldReturnSuccessCodeAndInfo()
#endif
        {
            server.ClearRequests();
            ...
            if (PubnubCommon.EnableStubTest)
            {
                Assert.Ignore("Ignored ThenWithAsyncSetRemoveUuidMetadataShouldReturnSuccessCodeAndInfo");
                return;
            }
            ...
            if (!receivedMessage)
            {
                Assert.IsTrue(receivedMessage, "SetUuidMetadata/SetChannelMetadata/Member AddUpdateRemove Failed");
            }
        ...
        [Test]
        public static void ThenMemberSetRemoveShouldReturnEventInfo()
        {
            server.ClearRequests();

            if (PubnubCommon.EnableStubTest)
            {
                Assert.Ignore("Ignored ThenMemberSetRemoveShouldReturnEventInfo");
                return;
            }
            ...
            if (receivedMessage)
            {
                receivedMessage = false;
                manualEvent = new ManualResetEvent(false);
                #region "ManageMembers Update/Remove"
                if (!string.IsNullOrEmpty(config.SecretKey))
                {
                    System.Diagnostics.Debug.WriteLine("pubnub.ManageMembers() UPDATE/REMOVE STARTED");
                    pubnub.ManageChannelMembers().Channel(channelMetadataId)
                        .Set(new List<PNChannelMember>()
                                {
                            new PNChannelMember() { Uuid = uuidMetadataId1, Custom = new Dictionary<string, object>(){ { "color", "green1" } } }
            ...
            Assert.IsTrue(receivedDeleteEvent && receivedSetEvent, "ManageMembers events Failed");
        ...
        [Test]
#if NET40
        public static void ThenWithAsyncMemberSetRemoveShouldReturnEventInfo()
#else
        public static async Task ThenWithAsyncMemberSetRemoveShouldReturnEventInfo()
#endif
        {
            server.ClearRequests();
